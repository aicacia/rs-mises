...
use crate::oidc_service::authorization_code::get_and_delete_authorization_code;

const ACCESS_TOKEN_EXPIRY: i64 = 3600;
const REFRESH_TOKEN_EXPIRY: i64 = 86400 * 30;
const JWT_SECRET: &[u8] = b"change_me_in_production";
  ...
  S: KeyValueStoreExecutor,
{
  match req.grant {
    Some(mises_proto::token_request::Grant::Password(password_grant)) => {
      handle_password_grant(repo, password_grant).await
    }
    Some(mises_proto::token_request::Grant::AuthorizationCode(authorization_code)) => {
      handle_authorization_code_grant(store, authorization_code).await
    }
...
async fn handle_authorization_code_grant<S>(
  store: &S,
  ...
  R: Repository + Clone + Send + Sync + 'static,
{
  ...
  let user_node = find_user_by_name(repo, &grant.username).await?;
...
async fn find_user_by_name<R>(repo: &R, username: &str) -> Result<R::Node, Status>
where
  R: Repository + Clone + Send + Sync + 'static,
{
  ...
  for el in elements {
    if let Element::Node(node) = el
      && let mises_core::model::node::NodeMeta::Identity(IdentityMeta::User { name, .. }) =
        &node.metadata
      && name == username
    {
      return Ok(node);
